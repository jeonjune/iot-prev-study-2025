...
def set_address() :
    try :
        name, phone, email, addr = input('정보 입력(이름, 휴대폰번호, 이메일, 주소 순) 하시오. > ').split(',')    
...
def save_contact(lst_contact) :
    ...
    f.close() # 파일을 열었으면, 끝난 후 반드시(!) 닫아줘야함.
...
def load_contact(lst_contact) :
    ...
    while True :
        ...
        # 25.01.16. 15:52 jiyoun.
        #\n으로 오류발생. \n을 리스트 저장 전에 삭제해야 함.
        lines = line.replace('\n','').split('/') # lines는 4개의 문자열을 가지는 리스트가 됨.
        # lines = ['지윤', '1972', 'naver.com', '부산']
        # lines[0] == '지윤'
...
if __name__ == '__main__' :
    ...
    # print('주소록 프로그램!')
    # 3. 클래스 실행 - 객체를 생성
    # first = contact.Contact('전지윤', '010-5230-1972', 'jyjeon0831@naver.com', '부산')
    # print(first)
    run() # 제일 처음 실행되는 함수
